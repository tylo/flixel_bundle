...
defaultProjectName="Flixel.tmproj";
defaultClassPath="org.flixel";
...
if [ -n "$fullProjectPath" ]; then
	...
	if [ -n "$fullClassPath" ]; then
		...
		mkdir -p "$projectPath/$projectName/bin/assets/common/css";
		mkdir -p "$projectPath/$projectName/bin/assets/common/js";
		...
		perl -pe 's/\$\{([^}]*)\}/$ENV{$1}/g' < "index.html" > "$projectPath/$projectName/bin/index.html";
		
		# For the debug html version of the modify the file name.
		TM_NEW_FILE_BASENAME="$projectName-debug";
		
		perl -pe 's/\$\{([^}]*)\}/$ENV{$1}/g' < "index.html" > "$projectPath/$projectName/bin/index-debug.html";

		# Copy static files.		
		cp "main.css" "$projectPath/$projectName/bin/assets/common/css/main.css";
		cp "swfaddress.js" "$projectPath/$projectName/bin/assets/common/js/swfaddress.js";
		cp "swfobject.js" "$projectPath/$projectName/bin/assets/common/js/swfobject.js";
